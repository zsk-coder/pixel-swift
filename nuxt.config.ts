...
export default defineNuxtConfig({
  ...
  // ── Sitemap Config ──
  // SEO 止血策略：收缩 sitemap 暴露面，集中域名权重到主语言
  // 1. 剔除所有语种的功能页（login、auth callback —— 没有 SEO 价值）
  // 2. 剔除小语种（非 en/zh）的工具页和辅助页（这些只是 UI 翻译，内容过于稀薄）
  // 3. 保留小语种的博客页面（博客有实际翻译的长文内容）
  // 4. 当小语种工具页补充了深度内容后，可逐步从此列表中移除，重新开放索引
  sitemap: {
    exclude: [
      // ── 所有语种的博客子路由排除（这些被 _sitemap-urls.ts 以标准路径动态生成） ──
      "/blog/en/**",
      "/blog/zh/**",
      "/blog/es/**",
      "/blog/fr/**",
      ...
      // ── 功能页：所有语种都不应出现在 sitemap 中 ──
      "/login",
      "/*/login",
      "/auth/**",
      "/*/auth/**",

      // ── 小语种（非 en/zh）工具页和辅助页：暂时剔除，集中权重 ──
      // 这些页面只有 UI 翻译文案，缺乏深度内容支撑
      // 注意：保留小语种博客页（/xx/blog/**），因为博客有真实翻译内容
      "/es",
      "/es/converter",
      "/es/compress-image",
      "/es/resize-image",
