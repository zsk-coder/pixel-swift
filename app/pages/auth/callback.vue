...
const copy = computed(() => ({
  title: tt("auth.callback.title", "Signing you in"),
  description: tt("auth.callback.description", "We're signing you in and getting your workspace ready."),
  failed: tt("auth.callback.failed", "We could not finish your sign-in. Please try again."),
  retry: tt("auth.callback.retry", "Try Google sign-in again"),
  home: tt("auth.callback.home", "Return home"),
  unavailable: tt("auth.callback.unavailable", "Supabase is not configured yet. Add the required environment variables to enable sign-in."),
}));
...
<template>
  <section class="mx-auto flex w-full justify-center px-4 sm:px-6 lg:px-8">
    <div class="w-full max-w-lg rounded-[28px] border border-slate-200 bg-white p-8 text-center shadow-[0_18px_48px_rgba(15,23,42,0.08)] dark:border-slate-700 dark:bg-slate-900">
      <div class="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-blue-50 text-primary dark:bg-blue-500/10 dark:text-blue-200">
        <span class="material-symbols-outlined text-[28px]" :class="{ 'animate-spin': isLoading }">
      ...
      <div v-if="errorMessage" class="mt-6 flex flex-col gap-3 sm:flex-row sm:justify-center">
        <NuxtLink to="/login" class="inline-flex items-center justify-center rounded-2xl bg-primary px-5 py-3 text-sm font-semibold text-white transition hover:bg-blue-700">
          {{ copy.retry }}
        </NuxtLink>
        <NuxtLink
          to="/"
